...
#define WATER pair<int, int>(-1, -1)
    ...
    void merge(int x, int y, int j, int k, int& nSet, vector<vector<int>>& rank, vector<vector<pair<int, int>>>& parent) {
        pair<int, int> parL = find(x, y, parent);
        pair<int, int> parR = find(j, k, parent);
        if(parL != parR) {
            nSet--;
            if(rank[parL.first][parL.second] < rank[parR.first][parR.second]) {
                parent[parL.first][parL.second] = parent[parR.first][parR.second];
            } else if(rank[parL.first][parL.second] > rank[parR.first][parR.second]) {
                parent[parR.first][parR.second] = parent[parL.first][parL.second];
            } else {
                parent[parL.first][parL.second] = parent[parR.first][parR.second];
                rank[parR.first][parR.second]++;
            }
        }
    }
            ...
            for(int k = 0; k < 4; ++k) {
                ...
                if(newX >= 0 and newX < m and newY >= 0 and newY < n) {
                    if(find(newX, newY, parent) != WATER) {
                        merge(x, y, newX, newY, nSet, rank, parent);
                    }
                }
            }
