...
public:
    ...
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        if(total & 1) {
            return findMedianSortedArrays(nums1, 0, nums2, 0, total / 2 + 1);
        }
        return (findMedianSortedArrays(nums1, 0, nums2, 0, total / 2) + findMedianSortedArrays(nums1, 0, nums2, 0, total / 2 + 1)) / 2.0;
    }
};